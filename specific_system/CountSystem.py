...
class CountSystem(GeneralSystem):
    def __init__(self, config_path):
        ...
        self.head_count = HeadCount("./config/config_head_count.json")
...
class HeadCount:
    ...
    def get_zone_dets_paint_point(self, preds, original_frame):
        ...
        for x1, y1, x2, y2, conf, cls_id in preds:
            ...
            if self.recty1 <= midy <= self.recty2 and self.rectx1 <= midx <= self.rectx2:
                ...
                cv2.circle(original_frame, center=(int(midx), int(
                    midy)), radius=1, color=(0, 0, 255), thickness=2)
    ...
    def process_frames(self, original_frame, preds):
        ...
        for x3, y3, x4, y4, track_id, conf, cls, det_ind in tracking_output:
            # Draw tracked ID
            cv2.putText(original_frame, str(int(track_id)), (int(x3 + 1), int(y3 + 1)), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (55, 255, 0), 2)
        ...
        for line in self.entrance_lines:
            cv2.line(original_frame, (line[0][0], line[0][1]),
                        (line[1][0], line[1][1]), (255, 255, 0), 2)

        # # --- Draw Timed Counts ---
        # if self.show_display_counts:
        ...
        return original_frame, total_in, total_out
